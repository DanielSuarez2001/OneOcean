...
let exportedMethods = {
    ...
    //Added Swim Season length
    validateSwimSeasonLength(swimSeasonLength) {
        if (swimSeasonLength === undefined || swimSeasonLength === null || swimSeasonLength === '') {
            return "Year-round";
        }
        if (typeof swimSeasonLength !== 'string') {
            throw 'swimSeasonLength must be a string';
        }
        let swimSeasonSanitized = swimSeasonLength.trim();
        if (swimSeasonSanitized.length === 0) {
            return "Year-round";
        }
        return swimSeasonSanitized;
    },
